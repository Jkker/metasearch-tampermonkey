...
export const config: Config = {
  engines: [
    ...
    },
    {
      ...
      matcher:
        /^https?:\/\/www\.google(?:\.[A-z]{2,3}){1,2}\/[^?]+\?(?!tbm=)(?:&?q=|(?:[^#](?!&tbm=))+?&q=)(?:.(?!&tbm=))*$|(^https?:\/\/xn--flw351e\.ml\/search\?q=)/i,
      embeddable: true,
      priority: 10,
    ...
    },
    {
      ...
      q: (url, query) =>
        decodeURIComponent(query.get('q') ?? '').replace?.(
          'site:reddit.com',
          ''
        ),
      embeddable: true,
    ...
    },
    {
      ...
      q: (url, query) => {
        if (url.match(/^https?:\/\/s\.weibo\.com\/weibo/i)) {
          return query.get('q') ?? '';
        }
        return (
          new URLSearchParams(
